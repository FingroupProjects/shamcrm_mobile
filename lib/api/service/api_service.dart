...
// ОТЛОЖЕННЫЙ ТОКЕН — ОДИН РАЗ, НАДЁЖНО
  ...
  Future<bool> deleteVoipToken({String? voipToken}) async {
    try {
      ...
      // Some existing Laravel installations expose this action as POST even
      // though it deletes a token. Support both route definitions during
      // rollout without sending two successful deletion requests.
...
// API Service
  Future<List<Task>> getTasks(
    ...
    if (taskStatusId != null && statuses == null) {
    ...
    if (projectId != null && !path.contains('/task/get-by-project/')) {
    ...
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      if (data['result']['data'] != null) {
        // При фильтрации статус передаётся через `statuses`, а не через
        // `taskStatusId`. Сохраняем его в модели, чтобы UI не отфильтровал
        // полученные задачи как задачи с неизвестным статусом.
...
// Метод для получения статусов задач
  Future<List<TaskStatus>> getTaskStatuses({
    ...
    try {
      ...
      if (projectId != null) {
        path = '/task-status/get-by-project/$projectId';
      }
      path = await _appendQueryParams(path);
      ...
      // Проектные статусы не используют analytics cache и не получают
      // дополнительные фильтры аналитики от другого экрана.
      ...
      if (response.statusCode == 200) {
        ...
        if (statusList != null && statusList.isNotEmpty) {
          // Проектные статусы не сохраняем в общий кэш организации.
          if (projectId == null) {
