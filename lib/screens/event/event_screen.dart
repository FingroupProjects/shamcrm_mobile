...
class _EventScreenState extends State<EventScreen>
  ...
  void initState() {
    ...
    context
        ...
        .add(FetchSalesFunnels()); // Загружаем воронки
    ...
    _tabController.addListener(() {
      if (!_tabController.indexIsChanging) {
        ...
        if (_skipNextTabListener) {
          debugPrint(
              'EventScreen: TabController listener - SKIPPED (filter just applied)');
          setState(() {
  ...
  Future<void> _fetchTutorialProgress() async {
    try {
      ...
      bool isTutorialShown =
          prefs.getBool('isTutorialShownNoticeIndex') ?? false;
      setState(() {
  ...
  void showTutorial() async {
    ...
    switch (_tutorialStep) {
      case 0: // Поиск
        currentTargets =
            targets.where((t) => t.identify == "EventSearchIcon").toList();
        break;
      case 1: // Фильтр
        currentTargets =
            targets.where((t) => t.identify == "EventFiltrIcon").toList();
        break;
      case 2: // Статусы
        currentTargets = targets
            .where((t) =>
                t.identify == "InProgressTab" || t.identify == "CompletedTab")
            .toList();
        break;
      case 3: // Карточка или Кнопка "Добавить"
        if (_eventBloc.state is EventDataLoaded) {
          ...
          if (events.isEmpty && _hasPermissionToAddEvent) {
            ...
            currentTargets =
                targets.where((t) => t.identify == "EventCard").toList();
          }
        }
        break;
...
// Новый метод для построения заголовка с выбором воронки
              ...
              Expanded(
                child: InkWell(
                  onTap: () async {
                    ...
                    if (selected != null) {
                      try {
                        await _apiService.saveSelectedEventSalesFunnel(
  ...
  Future<void> _handleManagerSelected(Map managers) async {
    ...
    debugPrint(
        'EventScreen: _handleManagerSelected - Dispatched FetchEventsWithFilters');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider.value(
            value:
                context.read<SalesFunnelBloc>()), // Добавляем SalesFunnelBloc
      ],
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: CustomAppBar(
            ...
            titleWidget: isClickAvatarIcon
                ...
                : _buildTitleWidget(
                    context), // Используем новый виджет заголовка
                        ...
                        child: RefreshIndicator(
                          ...
                          child: BlocListener<EventBloc, EventState>(
                            listener: (context, state) {
                              debugPrint(
                                  'EventScreen: BlocListener - state: ${state.runtimeType}');
                              if ((state is EventDataLoaded ||
                                      state is EventError) &&
                                  mounted &&
                                  (_isFilterLoading || _shouldShowLoader)) {
                                debugPrint(
                                    'EventScreen: Resetting loader flags');
                                setState(() {
                                  _isFilterLoading = false;
                                  _shouldShowLoader = false;
                                });
