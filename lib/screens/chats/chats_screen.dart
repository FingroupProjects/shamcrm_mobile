...
class _ChatsScreenState extends State<ChatsScreen> {
  ...
  void _initializeFirebase() async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (chatId != null) {
        ...
        checkChatExists(chatId).then((chatExists) {
          if (chatExists) {
            print('Чат с id $chatId найден. Открываем его.');
            _buildChatItemById(chatId).then((chatItem) {
              if (chatItem != null) {
                apiService.getMessages(chatId).then((messages) {
                  if (mounted) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
              ...
              if (mounted) {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) => ChatSmsScreen(
                      chatId: chatId,
                      chatItem: _buildChatItem(chatId),
                      messages: messages,
                    ),
                  ),
                );
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Chats>>(
        ...
        builder: (context, snapshot) {
          ...
          // Вывод информации о каждом чате в консоль
          for (var chat in chats) {
            print('ID чата: ${chat.id}, Имя: ${chat.name}');  // Вывод информации о чате
          }
  ...
  Future<ChatItem> _buildChatItemById(var chatId) async {
...
//       if (chatId != null) {
//         print('Получено уведомление для чата с id: $chatId');
//         checkChatExists(chatId).then((chatExists) async {
//           if (chatExists) {
//             print('Чат с id $chatId найден. Открываем его.');
