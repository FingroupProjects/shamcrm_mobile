...
class TaskCopyScreen extends StatefulWidget {
  ...
  const TaskCopyScreen({Key? key, required this.task, required this.statusId}) : super(key: key);

  @override
  _TaskCopyScreenState createState() => _TaskCopyScreenState();
}

class _TaskCopyScreenState extends State<TaskCopyScreen> {
  ...
  void _initializeFields() {
    ...
    setState(() {
      ...
      startDateController.text = task.startDate != null && task.startDate!.isNotEmpty
          ? DateFormat('dd/MM/yyyy').format(DateTime.parse(task.startDate!))
          : DateFormat('dd/MM/yyyy').format(DateTime.now());
      endDateController.text = task.endDate != null && task.endDate!.isNotEmpty
  ...
  void _fetchAndAddCustomFields() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Ошибка при загрузке кастомных полей!"),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Transform.translate(
          ...
          child: Text(
            '${AppLocalizations.of(context)!.translate('copy_task')} №${widget.task.taskNumber ?? ""}',
