  ...
  Future<void> _loadSelectedOrganization() async {
    ...
    void _loadUserPhone() async {
      ...
      try {
        ...
        // Находим самый длинный подходящий код страны
        for (var code in countryCodes) {
          if (phoneNumber.startsWith(code) && code.length > matchedCode.length) {
            matchedCode = code;
          }
        }
        
        if (matchedCode.isNotEmpty) {
          selectedDialCode = matchedCode;
          // Убираем код страны из номера и сохраняем остальные цифры
          phoneController.text = phoneNumber.substring(matchedCode.length);
        } else {
          // Если код страны не найден, сохраняем весь номер
          phoneController.text = phoneNumber;
          selectedDialCode = '+7'; // Значение по умолчанию
        }
      });
    } catch (e) {
      print('Ошибка при загрузке данных из API: $e');
