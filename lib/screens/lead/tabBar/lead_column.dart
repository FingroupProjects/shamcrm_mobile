                          ...
                          itemBuilder: (context, index) {
                            return Padding(
                              ...
                              child: LeadCard(
                                ...
                                // В LeadColumn, в методе build, в onStatusUpdated:
...
onStatusUpdated: () async {
  ...
  if (newStatusId != widget.statusId) {
    ...
    // КРИТИЧНО: Обновляем постоянные счетчики напрямую
    await LeadCache.updateLeadCountTemporary(widget.statusId, newStatusId);
    
    // Восстанавливаем счетчики из постоянного кэша через событие
    final currentBloc = BlocProvider.of<LeadBloc>(context);
    currentBloc.add(RestoreCountsFromCache());
  }
  
  // НЕ вызываем FetchLeads - это предотвращает перезапись счетчиков значениями пагинации
},
