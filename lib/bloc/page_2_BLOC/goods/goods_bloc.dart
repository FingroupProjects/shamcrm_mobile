...
class GoodsBloc extends Bloc<GoodsEvent, GoodsState> {
  ...
  Future<void> _fetchGoods(FetchGoods event, Emitter<GoodsState> emit) async {
    ...
    try {
      ...
      emit(GoodsError(
          friendlyError(e, fallback: 'Не удалось загрузить товары')));
    }
  }
  ...
  Future<void> _searchGoods(SearchGoods event, Emitter<GoodsState> emit) async {
    ...
    try {
      ...
      emit(GoodsError(
          friendlyError(e, fallback: 'Не удалось выполнить поиск товаров')));
    }
  }
  ...
  Future<void> _filterGoods(FilterGoods event, Emitter<GoodsState> emit) async {
    ...
    try {
      ...
      emit(GoodsError(
          friendlyError(e, fallback: 'Не удалось применить фильтры')));
    }
  }
  ...
  Future<void> _createGoods(CreateGoods event, Emitter<GoodsState> emit) async {
    ...
    try {
      ...
      emit(GoodsError(
          friendlyError(e, fallback: 'Ошибка при создании товара')));
    }
  }
  ...
  Future<void> _updateGoods(UpdateGoods event, Emitter<GoodsState> emit) async {
    ...
    try {
      ...
      emit(GoodsError(
          friendlyError(e, fallback: 'Не удалось обновить товар')));
    }
  }
  ...
  Future<void> _searchGoodsByBarcode(
    ...
    try {
      ...
      emit(GoodsBarcodeSearchResult(
        goods: [],
        error: friendlyError(e, fallback: 'Не удалось выполнить поиск'),
      ));
    }
  }
}
