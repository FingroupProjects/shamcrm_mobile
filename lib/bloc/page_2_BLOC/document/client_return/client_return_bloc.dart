...
class ClientReturnBloc extends Bloc<ClientReturnEvent, ClientReturnState> {
  ...
  ClientReturnBloc(this.apiService) : super(ClientReturnInitial()) {
    ...
    on<MassDisapproveClientReturnDocuments>(
        _onMassDisapproveClientReturnDocuments);
  ...
  _onFetchData(
      FetchClientReturns event, Emitter<ClientReturnState> emit) async {
  ...
  _delete(
    ...
    try {
      ...
      if (result['result'] == 'Success') {
        ...
        emit(ClientReturnDeleteSuccess('Документ успешно удален',
    ...
    if (_allData.isNotEmpty) {
      ...
      emit(ClientReturnLoaded(
        ...
        hasReachedMax: state is ClientReturnLoaded
            ? (state as ClientReturnLoaded).hasReachedMax
            : false,
        selectedData: selectedDocuments,
      ));
  ...
  Future<void> _onMassApproveClientReturnDocuments(
      MassApproveClientReturnDocuments event,
      Emitter<ClientReturnState> emit) async {
  ...
  Future<void> _onMassDisapproveClientReturnDocuments(
      MassDisapproveClientReturnDocuments event,
      Emitter<ClientReturnState> emit) async {
    final ls = _selectedDocuments
    ...
    try {
      ...
      emit(const ClientReturnDisapproveMassSuccess(
  ...
  Future<void> _onMassDeleteClientReturnDocuments(
      MassDeleteClientReturnDocuments event,
      Emitter<ClientReturnState> emit) async {
    final ls = _selectedDocuments
  ...
  Future<void> _onMassRestoreClientReturnDocuments(
      MassRestoreClientReturnDocuments event,
      Emitter<ClientReturnState> emit) async {
    final ls = _selectedDocuments
        .where((e) => e.deletedAt != null)
        .map((e) => e.id!)
  ...
  Future<void> _onUnselectAllDocuments(UnselectAllClientReturnDocuments event,
      Emitter<ClientReturnState> emit) async {
    _selectedDocuments = [];
  ...
  _onRestoreClientReturnDocument(RestoreClientReturnDocument event,
      Emitter<ClientReturnState> emit) async {
    emit(ClientReturnRestoreLoading());

    try {
      final result =
          await apiService.restoreClientReturnDocument(event.documentId);
      if (result['result'] == 'Success') {
        await Future.delayed(const Duration(milliseconds: 100));
        emit(const ClientReturnRestoreSuccess('Документ успешно восстановлен'));
      } else {
        emit(
            const ClientReturnRestoreError('Не удалось восстановить документ'));
      }
    } catch (e) {
      if (e is ApiException) {
