...
class SupplierReturnBloc
    extends Bloc<SupplierReturnEvent, SupplierReturnState> {
  ...
  SupplierReturnBloc(this.apiService) : super(SupplierReturnInitial()) {
    ...
    on<MassApproveSupplierReturnDocuments>(
        _onMassApproveSupplierReturnDocuments);
    on<MassDisapproveSupplierReturnDocuments>(
        _onMassDisapproveSupplierReturnDocuments);
    on<MassDeleteSupplierReturnDocuments>(_onMassDeleteSupplierReturnDocuments);
    on<MassRestoreSupplierReturnDocuments>(
        _onMassRestoreSupplierReturnDocuments);

    on<SelectSupplierReturnDocument>(_onSelectDocument);
    on<UnselectAllSupplierReturnDocuments>(_onUnselectAllDocuments);
  }

  Future<void> _onRestoreSupplierReturn(
      RestoreSupplierReturn event, Emitter<SupplierReturnState> emit) async {
    emit(SupplierReturnRestoreLoading());
    try {
      final result =
          await apiService.restoreSupplierReturnDocument(event.documentId);
      ...
      if (e is ApiException) {
        emit(SupplierReturnRestoreError(
  ...
  Future<void> _onFetchSupplierReturn(
      FetchSupplierReturn event, Emitter<SupplierReturnState> emit) async {
    if (event.forceRefresh) {
      _currentPage = 1;
      _allData = [];
      _filters = event.filters ?? {};
      emit(SupplierReturnLoading());
    } else if (state is SupplierReturnLoaded &&
        (state as SupplierReturnLoaded).hasReachedMax) {
      return;
    }
  ...
  Future<void> _onCreateSupplierReturn(
      CreateSupplierReturn event, Emitter<SupplierReturnState> emit) async {
  ...
  Future<void> _onUpdateSupplierReturn(
      UpdateSupplierReturn event, Emitter<SupplierReturnState> emit) async {
  ...
  Future<void> _onDeleteSupplierReturn(
      DeleteSupplierReturn event, Emitter<SupplierReturnState> emit) async {
    ...
    try {
      final result =
          await apiService.deleteSupplierReturnDocument(event.documentId);
    ...
    if (_allData.isNotEmpty) {
      ...
      emit(SupplierReturnLoaded(
        ...
        hasReachedMax: state is SupplierReturnLoaded
            ? (state as SupplierReturnLoaded).hasReachedMax
            : false,
        selectedData: selectedDocuments,
      ));
  ...
  Future<void> _onMassApproveSupplierReturnDocuments(
      MassApproveSupplierReturnDocuments event,
      Emitter<SupplierReturnState> emit) async {
    ...
    if (ls.isEmpty) {
      emit(const SupplierReturnApproveMassSuccess(
          "Нет документов для одобрения"));
      return;
    }
  ...
  Future<void> _onMassDisapproveSupplierReturnDocuments(
      MassDisapproveSupplierReturnDocuments event,
      Emitter<SupplierReturnState> emit) async {
    final ls = _selectedDocuments
    ...
    if (ls.isEmpty) {
      emit(const SupplierReturnDisapproveMassSuccess(
          "Нет документов для отмены одобрения"));
      return;
    }
  ...
  Future<void> _onMassDeleteSupplierReturnDocuments(
      MassDeleteSupplierReturnDocuments event,
      Emitter<SupplierReturnState> emit) async {
    final ls = _selectedDocuments
    ...
    if (ls.isEmpty) {
      emit(
          const SupplierReturnDeleteMassSuccess("Нет документов для удаления"));
      return;
    }
  ...
  Future<void> _onMassRestoreSupplierReturnDocuments(
      MassRestoreSupplierReturnDocuments event,
      Emitter<SupplierReturnState> emit) async {
    final ls = _selectedDocuments
        .where((e) => e.deletedAt != null)
        .map((e) => e.id!)
    ...
    if (ls.isEmpty) {
      emit(const SupplierReturnRestoreMassSuccess(
          "Нет документов для восстановления"));
      return;
    }
  ...
  // Выбор документов
  Future<void> _onSelectDocument(SelectSupplierReturnDocument event,
      Emitter<SupplierReturnState> emit) async {
  ...
  Future<void> _onUnselectAllDocuments(UnselectAllSupplierReturnDocuments event,
      Emitter<SupplierReturnState> emit) async {
    _selectedDocuments = [];
