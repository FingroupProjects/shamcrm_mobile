...
class Message {
  ...
  Message copyWith({
    ...
    List<MessageReaction>? reactions, // Реакции
  }) {
    return Message(
      id: id ?? this.id,
      text: text ?? this.text,
      ...
      reactions: reactions ?? this.reactions, // Реакции
    );
  }
  ...
  factory Message.fromJson(Map<String, dynamic> json, {String? chatType}) {
    ...
    if (json['sender'] != null && json['sender']['type'] != null) {
      ...
      if (senderType == 'lead') {
        ...
        isMyMessage = false;
        debugPrint(
            '🎯 [LEAD CHAT] sender.type=lead (клиент: $senderName) → isMyMessage=FALSE (слева)');
      } else if (senderType == 'user' && effectiveChatType == 'lead') {
        // ✅ Если отправитель - USER и чат лидный
        isMyMessage = true;
        debugPrint(
            '🎯 [LEAD CHAT] sender.type=user + chatType=lead (менеджер: $senderName) → isMyMessage=TRUE (справа)');
      } else {
        // ✅ Корпоративный чат или задача - используем стандартную логику
        isMyMessage = json['is_my_message'] ?? false;
        debugPrint(
            '🎯 [CORPORATE/TASK] sender.type=$senderType, chatType=$effectiveChatType → is_my_message=$isMyMessage (с сервера)');
      }
    } else {
      // Нет sender.type - используем стандартную логику
      isMyMessage = json['is_my_message'] ?? false;
      debugPrint(
          '🎯 [FALLBACK] НЕТ sender.type → is_my_message=$isMyMessage (с сервера)');
    }
    ...
    return Message(
        ...
        reactions: reactionsList, // Реакции
        duration: Duration(
