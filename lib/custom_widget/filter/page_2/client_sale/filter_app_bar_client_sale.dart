...
class ClientSaleFilterScreen extends StatefulWidget {
  ...
  @override
  _ClientSaleFilterScreenState createState() => _ClientSaleFilterScreenState();
}

class _ClientSaleFilterScreenState extends State<ClientSaleFilterScreen> {
  ...
  Future<void> _loadFilterState() async {
    try {
      ...
      if (mounted) {
        setState(() {
          final fromDateMillis = prefs.getInt('client_sale_from_date');
          final toDateMillis = prefs.getInt('client_sale_to_date');
          ...
          final leadName = prefs.getString('client_sale_cash_register');
          final leadId = prefs.getInt('client_sale_cash_register_id');
  ...
  Future<void> _saveFilterState() async {
    try {
      ...
      if (_selectedSupplier != null) {
        await prefs.setString('client_sale_supplier', _selectedSupplier!.name);
        await prefs.setInt('client_sale_supplier_id', _selectedSupplier!.id);
      } else {
        await prefs.remove('client_sale_supplier');
        await prefs.remove('client_sale_supplier_id');
      }
      ...
      if (_selectedLead != null) {
        await prefs.setString('client_sale_lead', _selectedLead!.name);
        await prefs.setInt('client_sale_lead_id', _selectedLead!.id);
      } else {
        await prefs.remove('client_sale_lead');
        await prefs.remove('client_sale_lead_id');
      }

      if (_selectedStatus != null) {
        await prefs.setString('money_client_sale_status', _selectedStatus!);
      } else {
        await prefs.remove('money_client_sale_status');
      }

      if (selectedAuthor != null) {
        await prefs.setString('client_sale_author', selectedAuthor!.name);
        await prefs.setInt('client_sale_author_id', selectedAuthor!.id);
      } else {
        await prefs.remove('client_sale_author');
        await prefs.remove('client_sale_author_id');
      }

      if (_isDeleted != null) {
        await prefs.setBool('client_sale_is_deleted', _isDeleted!);
      } else {
        await prefs.remove('client_sale_is_deleted');
      }
    } catch (e) {
