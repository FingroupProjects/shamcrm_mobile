...
class DebtorsFilterScreen extends StatefulWidget {
  ...
  @override
  _DebtorsFilterScreenState createState() => _DebtorsFilterScreenState();
}
...
class _DebtorsFilterScreenState extends State<DebtorsFilterScreen> {
  ...
  Future<void> _loadFilterState() async {
    ...
    setState(() {
      ...
      final leadName = prefs.getString('debtors_lead');
      ...
      final supplierId = prefs.getInt('debtors_supplier_id');
      final supplierName = prefs.getString('debtors_supplier');
  ...
  Future<void> _saveFilterState() async {
    ...
    if (_toDate != null) {
      await prefs.setInt('debtors_to_date', _toDate!.millisecondsSinceEpoch);
    } else {
      await prefs.remove('debtors_to_date');
    }
    await prefs.setString('debtors_amount_from', _amountFromController.text);
    await prefs.setString('debtors_amount_to', _amountToController.text);
    if (_selectedLead != null) {
      await prefs.setString('debtors_lead', _selectedLead!.name);
      await prefs.setInt('debtors_lead_id', _selectedLead!.id);
    } else {
      await prefs.remove('debtors_lead');
      await prefs.remove('debtors_lead_id');
    }
    if (_selectedSupplier != null) {
      await prefs.setString('debtors_supplier', _selectedSupplier!.name);
      await prefs.setInt('debtors_supplier_id', _selectedSupplier!.id);
    } else {
      await prefs.remove('debtors_supplier');
      await prefs.remove('debtors_supplier_id');
    }
  }
