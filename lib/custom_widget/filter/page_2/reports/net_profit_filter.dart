...
class _NetProfitFilterScreenState extends State<NetProfitFilterScreen> {
  ...
  Future<void> _loadFilterState() async {
    ...
    setState(() {
      final fromDateMillis = prefs.getInt('net_profit_from_date');
      final toDateMillis = prefs.getInt('net_profit_to_date');
      ...
      _amountFromController.text = prefs.getString('net_profit_amount_from') ?? widget.initialAmountFrom ?? '';
      _amountToController.text = prefs.getString('net_profit_amount_to') ?? widget.initialAmountTo ?? '';
    });
  }
  ...
  Future<void> _saveFilterState() async {
    final prefs = await SharedPreferences.getInstance();
    if (_fromDate != null) {
      await prefs.setInt('net_profit_from_date', _fromDate!.millisecondsSinceEpoch);
    } else {
      await prefs.remove('net_profit_from_date');
    }
    if (_toDate != null) {
      await prefs.setInt('net_profit_to_date', _toDate!.millisecondsSinceEpoch);
    } else {
      await prefs.remove('net_profit_to_date');
    }
    await prefs.setString('net_profit_amount_from', _amountFromController.text);
    await prefs.setString('net_profit_amount_to', _amountToController.text);
  }
