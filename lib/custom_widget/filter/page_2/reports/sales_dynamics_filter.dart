...
class SalesDynamicsFilterScreen extends StatefulWidget {
  ...
  @override
  _SalesDynamicsFilterScreenState createState() => _SalesDynamicsFilterScreenState();
}
...
class _SalesDynamicsFilterScreenState extends State<SalesDynamicsFilterScreen> {
  ...
  Future<void> _loadFilterState() async {
    ...
    setState(() {
      final fromDateMillis = prefs.getInt('sales_dynamics_from_date');
      final toDateMillis = prefs.getInt('sales_dynamics_to_date');
      ...
      _amountFromController.text = prefs.getString('sales_dynamics_amount_from') ?? widget.initialAmountFrom ?? '';
      _amountToController.text = prefs.getString('sales_dynamics_amount_to') ?? widget.initialAmountTo ?? '';
    });
  }
  ...
  Future<void> _saveFilterState() async {
    final prefs = await SharedPreferences.getInstance();
    if (_fromDate != null) {
      await prefs.setInt('sales_dynamics_from_date', _fromDate!.millisecondsSinceEpoch);
    } else {
      await prefs.remove('sales_dynamics_from_date');
    }
    if (_toDate != null) {
      await prefs.setInt('sales_dynamics_to_date', _toDate!.millisecondsSinceEpoch);
    } else {
      await prefs.remove('sales_dynamics_to_date');
    }
    await prefs.setString('sales_dynamics_amount_from', _amountFromController.text);
    await prefs.setString('sales_dynamics_amount_to', _amountToController.text);
  }
