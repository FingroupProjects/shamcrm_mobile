...
class _TopSellingGoodsFilterScreenState extends State<TopSellingGoodsFilterScreen> {
  ...
  Future<void> _loadFilterState() async {
    ...
    setState(() {
      final fromDateMillis = prefs.getInt('top_selling_goods_from_date');
      final toDateMillis = prefs.getInt('top_selling_goods_to_date');
      ...
      _amountFromController.text = prefs.getString('top_selling_goods_amount_from') ?? widget.initialAmountFrom ?? '';
      _amountToController.text = prefs.getString('top_selling_goods_amount_to') ?? widget.initialAmountTo ?? '';
    });
  }
  ...
  Future<void> _saveFilterState() async {
    final prefs = await SharedPreferences.getInstance();
    if (_fromDate != null) {
      await prefs.setInt('top_selling_goods_from_date', _fromDate!.millisecondsSinceEpoch);
    } else {
      await prefs.remove('top_selling_goods_from_date');
    }
    if (_toDate != null) {
      await prefs.setInt('top_selling_goods_to_date', _toDate!.millisecondsSinceEpoch);
    } else {
      await prefs.remove('top_selling_goods_to_date');
    }
    await prefs.setString('top_selling_goods_amount_from', _amountFromController.text);
    await prefs.setString('top_selling_goods_amount_to', _amountToController.text);
  }
