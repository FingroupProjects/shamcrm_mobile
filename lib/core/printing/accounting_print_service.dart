...
class AccountingPrintService {
        ...
        pw.Container(
          ...
          child: pw.Column(
            ...
            children: [
              ...
              pw.SizedBox(height: 4),
              _smallMeta('Дата', _formatDate(document.date), settings),
              if (settings.showStatus) ...[
                pw.SizedBox(height: 4),
                _statusChip(document.statusText, settings),
              ],
            ],
          ),
        ),
        ...
        child: pw.Column(
          children: [
            if (settings.showTotalQuantity)
              _totalRow(
                'Общее количество',
                _formatNumber(document.totalQuantity),
                settings,
              ),
            if (settings.showTotalQuantity && settings.showSums)
    ...
    return pw.RichText(
      text: pw.TextSpan(
        children: [
          ...
          pw.TextSpan(
            text: value,
            style: pw.TextStyle(fontSize: _font(settings, 9)),
          ),
        ],
      ),
    );
  ...
  static pw.Widget _statusChip(String status, PrintTemplateSettings settings) {
    return pw.Container(
  ...
  static pw.Widget _totalRow(
    ...
    final style = pw.TextStyle(
      fontSize: _font(settings, bold ? 13 : 10),
  ...
  static PdfPageFormat _pageFormat(PrintTemplateSettings settings) {
    switch (settings.paperSize) {
      case 'receipt58':
        return PdfPageFormat(58 * PdfPageFormat.mm, 500 * PdfPageFormat.mm);
      case 'receipt80':
        return PdfPageFormat(80 * PdfPageFormat.mm, 500 * PdfPageFormat.mm);
      case 'custom':
