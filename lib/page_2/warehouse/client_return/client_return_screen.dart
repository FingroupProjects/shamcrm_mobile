...
class _ClientReturnScreenState extends State<ClientReturnScreen> {
  ...
  void initState() {
    ...
    _clientReturnBloc = context.read<ClientReturnBloc>()
  ...
  Future<void> _checkPermissions() async {
    try {
      final create =
          await _apiService.hasPermission('client_return_document.create');
      final update =
          await _apiService.hasPermission('client_return_document.update');
      final delete =
          await _apiService.hasPermission('client_return_document.delete');
        ...
        child: Scaffold(
                    ...
                    builder: (context, state) {
                      if (state is ClientReturnLoaded) {
                        ...
                        return AppBarSelectionMode(
                          title: localizations
                                  ?.translate('appbar_client_returns') ??
                              'Возврат от клиента',
                          onDismiss: () {
                            setState(() {
                          ...
                          onApprove: () {
                            ...
                            _clientReturnBloc
                                .add(MassApproveClientReturnDocuments());
                          },
                          onDisapprove: () {
                            setState(() {
                              _selectionMode = false;
                            ...
                            _clientReturnBloc
                                .add(MassDisapproveClientReturnDocuments());
                          },
                          onDelete: () {
                            setState(() {
                              _selectionMode = false;
                            ...
                            _clientReturnBloc
                                .add(MassDeleteClientReturnDocuments());
                          },
                          onRestore: () {
                            setState(() {
                              _selectionMode = false;
                            ...
                            _clientReturnBloc
                                .add(MassRestoreClientReturnDocuments());
                          },
                      ...
                      return AppBarSelectionMode(
                        title:
                            localizations?.translate('appbar_client_returns') ??
                                'Возврат от клиента',
                        onDismiss: () {
                          setState(() {
                            _selectionMode = false;
          ...
          body: BlocListener<ClientReturnBloc, ClientReturnState>(
            listener: (context, state) {
              ...
              if (state is ClientReturnLoaded) {
              ...
              } else if (state is ClientReturnCreateSuccess) {
                // НОВОЕ: Если state есть
                if (mounted) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    if (mounted && context.mounted) {
              ...
              } else if (state is ClientReturnUpdateSuccess) {
                // НОВОЕ
                if (mounted) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    if (mounted && context.mounted) {
                ...
                debugPrint(
                    "ClientReturnScreen.Bloc.State.ClientReturnRestoreSuccess: ${_clientReturnBloc.state}");
                if (mounted) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    if (mounted && context.mounted) {
                      _showSnackBar(state.message, true);
                    ...
                    if (mounted && context.mounted) {
                      if (state.statusCode == 409) {
                        showSimpleErrorDialog(
                          ...
                          state.message,
                          errorDialogEnum:
                              ErrorDialogEnum.clientReturnUnapprove,
                        );
                        return;
                      }
                      showCustomSnackBar(
            ...
            child: BlocBuilder<ClientReturnBloc, ClientReturnState>(
              builder: (context, state) {
                if (_isInitialLoad ||
                    state is ClientReturnLoading ||
                    state is ClientReturnDeleteLoading ||
                    state is ClientReturnRestoreLoading ||
                    state is ClientReturnApproveMassLoading ||
                    state is ClientReturnDisapproveMassLoading ||
                    state is ClientReturnDeleteMassLoading ||
                    state is ClientReturnRestoreMassLoading ||
                    _isRefreshing) {
                  return Center(
                ...
                if (currentData.isEmpty && state is ClientReturnLoaded) {
                  return _isSearching
                      ...
                      : HelpfulEmptyState.section(
                          ...
                          titleKey: 'empty_client_return_title',
                          subtitleKey: 'empty_client_return_subtitle',
                        );
                }
                          ...
                          ? Dismissible(
                              ...
                              onDismissed: (direction) {
                                ...
                                if (isDeleted) {
                                  ...
                                  _clientReturnBloc
                                      .add(RestoreClientReturnDocument(
                                  ...
                                  _clientReturnBloc
                                      .add(DeleteClientReturnDocument(
                                    currentData[index].id!,
                                    localizations!,
                                    shouldReload: true,
                                  ));
                              ...
                              child: ClipRRect(
                                ...
                                child: ClientReturnCard(
                                  ...
                                  onTap: () {
                                    if (_selectionMode) {
                                      _clientReturnBloc.add(
                                          SelectClientReturnDocument(
                                              currentData[index]));
                                    ...
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (ctx) => BlocProvider.value(
                                          value: _clientReturnBloc,
                                          child:
                                              ClientReturnDocumentDetailsScreen(
                                  ...
                                  isSelected: (state as ClientReturnLoaded)
                                          .selectedData
