...
class CreateManufactureDocumentScreen extends StatefulWidget {
  final int? organizationId;

  const CreateManufactureDocumentScreen({this.organizationId, super.key});

  @override
  CreateManufactureDocumentScreenState createState() =>
      CreateManufactureDocumentScreenState();
}

class CreateManufactureDocumentScreenState
    extends State<CreateManufactureDocumentScreen>
  ...
  void _createDocument({bool approve = false}) async {
    ...
    try {
      ...
      final bloc = context.read<ManufactureBloc>();
      bloc.add(CreateManufactureDocument(
    ...
    final result = await handleBarcodeForDocument(
      ...
      onItemAdded: (newItem) {
        ...
        setState(() {
          ...
          final itemWithMaterials = Map<String, dynamic>.from(newItem);
          itemWithMaterials['materials'] = _buildMaterialsFromItem(itemWithMaterials);
          _ensureMaterialControllersForItem(itemWithMaterials);
          _items.add(itemWithMaterials);
          final variantId = newItem['variantId'] as int;
...
// Обёртка для сохранения состояния вкладок
