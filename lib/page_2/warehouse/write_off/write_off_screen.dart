...
class _WriteOffScreenState extends State<WriteOffScreen> {
  ...
  void initState() {
    ...
    _writeOffBloc = context.read<WriteOffBloc>()
      ..add(FetchWriteOffs(
        forceRefresh: true,
      ));
  ...
  Future<void> _checkPermissions() async {
    try {
      final create =
          await _apiService.hasPermission('write_off_document.create');
      final update =
          await _apiService.hasPermission('write_off_document.update');
      final delete =
          await _apiService.hasPermission('write_off_document.delete');
                      ...
                      if (state is WriteOffLoaded) {
                        ...
                        return AppBarSelectionMode(
                          ...
                          onDisapprove: () {
                            ...
                            _writeOffBloc
                                .add(MassDisapproveWriteOffDocuments());
                          },
                          onDelete: () {
                            setState(() {
                              _selectionMode = false;
          ...
          body: BlocListener<WriteOffBloc, WriteOffState>(
            listener: (context, state) {
              ...
              if (state is WriteOffLoaded) {
                ...
                debugPrint(
                    "WriteOffScreen.Bloc.State.WriteOffDeleteSuccess: ${_writeOffBloc.state}");
                if (mounted) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    if (mounted && context.mounted) {
                      _showSnackBar(state.message, true);
                ...
                debugPrint(
                    "WriteOffScreen.Bloc.State.WriteOffRestoreSuccess: ${_writeOffBloc.state}");
                if (mounted) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    if (mounted && context.mounted) {
                      _showSnackBar(state.message, true);
            ...
            child: BlocBuilder<WriteOffBloc, WriteOffState>(
              builder: (context, state) {
                ...
                if (_isInitialLoad ||
                    state is WriteOffLoading ||
                    state is WriteOffDeleteLoading ||
                    state is WriteOffRestoreLoading ||
                    state is WriteOffCreateLoading ||
                    state is WriteOffApproveMassLoading ||
                    state is WriteOffDisapproveMassLoading ||
                    state is WriteOffDeleteMassLoading ||
                    state is WriteOffRestoreMassLoading ||
                    _isRefreshing) {
                  return Center(
                                ...
                                child: WriteOffCard(
                                  ...
                                  onTap: () {
                                    if (_selectionMode) {
                                      ...
                                      final currentState =
                                          context.read<WriteOffBloc>().state;
                                  ...
                                  isSelected: (state as WriteOffLoaded)
                                          .selectedData
                                  ...
                                  onLongPress: _hasDeletePermission
                                      ? () {
                                          ...
                                          _writeOffBloc.add(SelectDocument(
                                              currentData[index]));
                                        }
                                      : () {},
                                ),
