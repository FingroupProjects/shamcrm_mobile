...
class _EditClientSalesDocumentScreenState
  ...
  /// 🔍 Добавляет товар через штрих-код с quantity=1
  void _handleVariantSelectionFromBarcode(Map<String, dynamic> newItem) {
    ...
    setState(() {
      ...
      _priceFocusNodes[variantId] = FocusNode();
      _items.last['price'] = initialPrice;
      _items.last['total'] = initialPrice;
      _priceErrors[variantId] = false;
      _quantityErrors[variantId] = false;
      _collapsedItems[variantId] = false;
      if (!newItem.containsKey('amount')) {
...
// ✅ УЛУЧШЕНО: функция _updateItemQuantity
  void _updateItemQuantity(int variantId, String value) {
    final quantity = WarehouseValidationHelper.parseFlexibleDecimal(value);
    ...
    return AppBar(
      ...
      title: _isSearching
        ...
        children: [
          Expanded(
            child: Text(
              '${localizations.translate('edit_client_sale') ?? 'Редактировать'} №${widget.document.docNumber}',
              maxLines: 1,
