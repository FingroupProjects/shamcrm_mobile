  ...
  class OrderAddScreen extends StatefulWidget {
    ...
    final int? leadId; // Новый параметр
  ...
  class _OrderAddScreenState extends State<OrderAddScreen> {
    ...
    void initState() {
      super.initState();
      // Инициализация selectedLead, если leadId передан из OrdersWidget
    ...
    children: [
      ...
      if (widget.leadId == null) // Отображаем только если leadId не передан
        LeadWithManager(
      ...
      if (widget.leadId == null) const SizedBox(height: 8), // Отступ, если поле лида отображается
      ManagerForLead(
            ...
            onPressed: () async {
              if (_formKey.currentState!.validate() && _items.isNotEmpty) {
                ...
                orderBloc.add(CreateOrder(
                  phone: selectedDialCode!, // Используем только selectedDialCode
                  leadId:
                      widget.leadId ?? int.parse(selectedLead ?? '0'),
                  delivery: !isPickup,
                  deliveryAddress:
