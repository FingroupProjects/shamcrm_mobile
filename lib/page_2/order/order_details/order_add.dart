...
class _OrderAddScreenState extends State<OrderAddScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (selectedLead != null) {
        context.read<DeliveryAddressBloc>().add(FetchDeliveryAddresses(leadId: int.parse(selectedLead!)));
      }
    });
  }
  ...
  void _navigateToAddProduct() async {
    final Order tempOrder = widget.order ??
        Order(
          id: 0,
          phone: selectedDialCode != null ? '$selectedDialCode${_phoneController.text}' : _phoneController.text,
          orderNumber: '',
        ...
        body: BlocConsumer<OrderBloc, OrderState>(
          listener: (context, state) {
            if (state is OrderSuccess) {
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                setState(() {
                  _items = state.orderDetails!.goods.map((good) {
                    final imagePath = good.variant != null &&
                            good.variant!['good'] != null &&
                            good.variant!['good']['files'] != null &&
                            (good.variant!['good']['files'] as List).isNotEmpty
                  ...
                  _deliveryAddressController.text = state.orderDetails!.deliveryAddress ?? '';
                  selectedLead = state.orderDetails!.lead.id.toString();
                  _deliveryMethod = state.orderDetails!.delivery
                      ? AppLocalizations.of(context)!.translate('delivery')
                      : AppLocalizations.of(context)!.translate('self_delivery');
                  _selectedDeliveryAddress = state.orderDetails!.deliveryAddress != null
                      ? DeliveryAddress(
        ...
        children: [
          SizedBox(
            ...
            child: item['imagePath'] != null && baseUrl != null
                ? ClipRRect(
                    ...
                    child: Image.network(
                      item['imagePath'].startsWith('http') ? item['imagePath'] : '$baseUrl/${item['imagePath']}',
                      width: 48,
                      height: 48,
                      fit: BoxFit.cover,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  item['name'] ?? AppLocalizations.of(context)!.translate('no_name_chat'),
                  style: const TextStyle(
                    fontSize: 14,
                    fontFamily: 'Gilroy',
                    fontWeight: FontWeight.w500,
              ...
              onPressed: () async {
                if (_formKey.currentState!.validate() && _items.isNotEmpty) {
                  if (selectedManager == null) {
                    ...
                    showCustomSnackBar(
                      context: context,
                      message: AppLocalizations.of(context)!.translate('please_select_manager'),
                      isSuccess: false,
                    );
                    return;
                  }
                  if (_deliveryMethod == AppLocalizations.of(context)!.translate('self_delivery') && _selectedBranch == null) {
                    showCustomSnackBar(
                      context: context,
                  ...
                  if (_deliveryMethod == AppLocalizations.of(context)!.translate('delivery') && _selectedDeliveryAddress == null) {
                    showCustomSnackBar(
                      context: context,
