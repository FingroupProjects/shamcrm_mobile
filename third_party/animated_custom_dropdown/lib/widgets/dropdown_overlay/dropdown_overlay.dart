...
class _DropdownOverlay<T> extends StatefulWidget {
  ...
  final _PaginatedRequest<T>? paginatedRequest;
  ...
  final Widget? paginationLoadingIndicator;
  ...
  const _DropdownOverlay({
    ...
    required this.paginationLoadingIndicator,
  });
...
class _DropdownOverlayState<T> extends State<_DropdownOverlay<T>> {
  ...
  bool get _usesPaginatedRequest => widget.paginatedRequest != null;
  ...
  void initState() {
    ...
    hasMoreItems = _usesPaginatedRequest;
    scrollController.addListener(_handleScroll);

    if (_usesPaginatedRequest && widget.items.isEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
  ...
  Future<void> _performPaginatedSearch(String query) async {
    if (!_usesPaginatedRequest || isSearchRequestLoading) {
      return;
    }
    ...
    try {
      final response = await widget.paginatedRequest!(query, 1);
      if (!mounted) return;

      setState(() {
        items = response.items;
        hasMoreItems = response.hasMore;
        mayFoundSearchRequestResult = response.items.isNotEmpty;
  ...
  Future<void> _loadNextPage() async {
    ...
    try {
      final response = await widget.paginatedRequest!(currentQuery, nextPage);
      if (!mounted) return;

      final mergedItems = List<T>.from(items);
  ...
  void _handleScroll() {
    if (!_usesPaginatedRequest || !scrollController.hasClients) {
      return;
    }
